...
namespace SealTest
{
    ...
    public class SosiGwTest
    {
        ...
        {
            DoLogin(Global.MOCES_cpr_gyldig);

            var client = new MedicineCardPortTypeClient("SosiGWFMK");
            client.GetMedicineCard_20120101(MakeSecurity(assertion), header);
        }
        ...
        public void GatewaySecureBrowserLoginTest()
        {
            ...
            {
                OperationContext.Current.OutgoingMessageHeaders.Add(new SealCardMessageHeader(SealCard.Create(assertion)));
        ...
        public void DoLogin(X509Certificate2 cert)
        {
            ...
            header = MakeHeader();
            assertion = SealCard.Create(sec.Assertion).GetAssertion<Assertion>(typeof(GW.AssertionType).Name);
        }

        private GW.Security MakeSecurity(GW.AssertionType assertion)
        {
        ...
        private Security MakeSecurity(Assertion assertion)
        {
            return new Security
            {
                id = Guid.NewGuid().ToString("D"),
        ...
        private GW.AssertionType MakeAssertionForSTS()
        {
